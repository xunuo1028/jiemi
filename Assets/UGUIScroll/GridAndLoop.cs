   ...
   //     mChild.Sort(sortByName);//按照Item名字排序 
    ...
    void ResetChildPosition()
    {
        ...
        for (i = 0; i < imax; i++)
        {
            ...
            else cullContent=temp.gameObject.active = false;//如果预制Item数超过maxIndex则将超过部分隐藏 并 设置cullCintent为ufalse 并且不再更新 panel尺寸
    ...
    void WrapContent()
    {
        ...
        if (mHorizontal)
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos=temp.localPosition;
                    temp.gameObject.active = (pos.x>min&&pos.x<max)?true:false;
        ...
        else 
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos = temp.localPosition;
                    temp.gameObject.active = (pos.y > min && pos.y < max) ? true : false;
                }
